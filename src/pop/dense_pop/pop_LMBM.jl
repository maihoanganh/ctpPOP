function get_constant_trace_LMBM(x::Vector{PolyVar{true}},g::Vector{Polynomial{true,Float64}},h::Vector{Polynomial{true,Float64}},k::Int64;UseEq::Bool=true)
    n=length(x)
    m=length(g)
    l=length(h)
...
function model_POP_LMBM(x::Vector{PolyVar{true}},f::Polynomial{true,Float64},g::Vector{Polynomial{true,Float64}},h::Vector{Polynomial{true,Float64}},k::Int64;UseEq::Bool=true)
    ...
    a_val,a0,norm_a0,opnorm_a=rescale_LMBM(a_ind1,a_ind2,a_val,a_len,a0,zeta)
...
function rescale_LMBM(a_ind1::Vector{UInt64},a_ind2::Vector{UInt64},a_val::Vector{Float64},a_len::UInt64,a0::Vector{Float64},zeta::UInt64)
    ...
    opnorm_a=1.0#svds(sparse(a_ind1,a_ind2,a_val), nsv = 1)[1].S[1]
...
function SmallEig_LMBM(mat::Matrix{Float64},s::UInt64;EigAlg::String="Arpack")
    if s==1
    ...
    elseif EigAlg=="Mix"
       try 
           @fastmath @inbounds E=partialeigen(partialschur(mat, nev=1,tol=1e-5, which=SR())[1])
           return E[1][1],E[2][:,1]
       catch
           @fastmath @inbounds E=eigs(mat,nev = 1,which=:SR,tol=1e-5) 
           return E[1][1],E[2][:,1]
       end
    else
...
function SmallEig_block_LMBM(vec::Adjoint{Float64,Array{Float64,1}},sk::UInt64;EigAlg::String="Arpack")
    ...
    return SmallEig_LMBM(B,sk,EigAlg=EigAlg)
end


function SmallEigBlocks_LMBM(a0_block::Vector{Adjoint{Float64,Array{Float64,1}}},a_block::Vector{SparseMatrixCSC{Float64,UInt64}},y::Vector{Float64},s::Vector{UInt64},omega::Int64;EigAlg::String="Arpack")
    ...
    @fastmath @inbounds @simd for p in 1:omega
        eigval,eigvec=SmallEig_block_LMBM(a0_block[p]+y'*a_block[p],s[p],EigAlg=EigAlg)
...
function solve_POP_LMBM(omega::Int64,a0_block::Vector{Adjoint{Float64,Array{Float64,1}}},a_block::Vector{SparseMatrixCSC{Float64,UInt64}},s::Vector{UInt64},zeta::UInt64,norm_a0::Float64,opnorm_a::Float64,ak::Float64;EigAlg::String="Arpack",tol::Float64=1e-4)
    ...
    function phi(nvar::Cint,xp::Ptr{Cdouble},gp::Ptr{Cdouble})
        ...
        eigval,eigvec,ind=SmallEigBlocks_LMBM(a0_block,a_block,zvar,s,omega,EigAlg=EigAlg)
...
function POP_LMBM(x::Vector{PolyVar{true}},f::Polynomial{true},g::Vector{Polynomial{true,Float64}},h::Vector{Polynomial{true,Float64}},k::Int64;EigAlg="Arpack",tol=1e-4,UseEq=true)

    @time begin
    
    
    @time omega,a0_block,a_block,s,zeta,norm_a0,opnorm_a,ak,P,vsk,n,m,l=model_POP_LMBM(x,f,g,h,k,UseEq=UseEq)
    
 
    @time opt_val,z=solve_POP_LMBM(omega,a0_block,a_block,s,zeta,norm_a0,opnorm_a,ak,EigAlg=EigAlg,tol=tol)
    
    @time opt_sol=extract_optimizer_LMBM(a0_block[1]+z'*a_block[1],P,s[1],vsk,n,m,l,opt_val,f,g,h,x,EigAlg=EigAlg) 
       println("Total time:")
                    end
    return opt_val,opt_sol
end
                    
function get_Grammat_LMBM(vec::Adjoint{Float64,Array{Float64,1}},sk::UInt64,P::Vector{Float64};EigAlg="Arpack")
    t_iter=1
    ...
    eigval,~=SmallEig_LMBM(-Gr,sk,EigAlg=EigAlg)
...
function extract_optimizer_LMBM(vec::Adjoint{Float64,Array{Float64,1}},P::Vector{Float64},lu0::UInt64,basis_sigma0::Matrix{UInt64},n::Int64,l_g::Int64,l_h::Int64,opt_val::Float64,f::Polynomial{true},g::Vector{Polynomial{true,Float64}},h::Vector{Polynomial{true,Float64}},x::Vector{PolyVar{true}};EigAlg="Arpack")
