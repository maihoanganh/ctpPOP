function pop_opf_linear_cost(data::Dict{String, Any}; normalize = true)
    ...
    for (i,bus) in ref[:bus]
        ...
        add_constraint!( model,
                        ...
                        PolyPowerModels.fl_sum(shunt["gs"] for shunt in bus_shunts)*(vr[i]^2+vi[i]^2); normalize = true
                       )  
        add_constraint!( model,
                        ...
                        PolyPowerModels.fl_sum(shunt["bs"] for shunt in bus_shunts)*(vr[i]^2+vi[i]^2); normalize = true
                       )
