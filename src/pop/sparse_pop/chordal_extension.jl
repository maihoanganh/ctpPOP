...
function MCSM!(G)
    ...
    for i=n:-1:1
        ...
        for u in intersect(neighbors(G, v), unorder)
            push!(reach[w[u]], u)
            push!(S, u)
            deleteat!(unreach, bfindn(unreach, length(unreach), u))
        end
        for k=1:maxw-1
            while reach[k]!=[]
                z=pop!(reach[k])
...
function MinimalChordal!(G, order, F)
    ...
    for i=n:-1:1
        ...
        if length(ind)>0
            ...
            for j=1:size(cand,2)
                rem_edge!(W, bfindn(inc,linc,cand[1,j]), bfindn(inc,linc,cand[2,j]))
            end
            keep,_ = MCSM!(W)
            for j=1:size(cand,2)
                if !has_edge(keep, bfindn(inc,linc,cand[1,j]), bfindn(inc,linc,cand[2,j]))
...
function bfindn(A::Vector{T}, l::Int, a) where {T<:Number}
    if l==0
        return 0
    end
    low=1
