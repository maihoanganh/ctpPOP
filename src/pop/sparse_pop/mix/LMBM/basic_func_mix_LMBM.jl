function run_get_blocks_Cliq_mix_LMBM(n::Int64,p::Int64,t::Int64,k::Int64,lI::Vector{Int64},lJ::Vector{Int64},lW::Vector{Int64},I::Vector{Vector{UInt16}},J::Vector{Vector{UInt64}},W::Vector{Vector{UInt64}},Supp::SparseMatrixCSC{UInt64,Int64},IndA::Vector{Vector{UInt64}},lmon_g::Vector{UInt64},lmon_h::Vector{UInt64},supp_g::Array{SparseMatrixCSC{UInt64,Ti} where Ti<:Integer,1},supp_h::Array{SparseMatrixCSC{UInt64,Ti} where Ti<:Integer,1},coe_g::Vector{Vector{Float64}},coe_h::Vector{Vector{Float64}},dg::Vector{Int64},dh::Vector{Int64})
    ...
    for j in 1:p
        v[j],s2k[j],sk[j],sk_g[j],sk_h[j],s2k_g[j],s2k_h[j]=initial_mix_LMBM(lI[j],lJ[j],lW[j],k,dg[J[j]],dh[W[j]])
         

        Usupp[j],lUsupp[j],block_g0[j],block_g[j],block_h[j],lblock_g0[j],lblock_g[j],lblock_h[j],lt_block_g0[j],lt_block_g[j],lt_block_h[j]=get_blocks_Cliq_mix_LMBM(k,lI[j],lJ[j],lW[j],[Supp[I[j],IndA[j]] 2*v[j][:,1:sk[j]]],lmon_g[J[j]],lmon_h[W[j]],[supp_g[i][I[j],:] for i in J[j]],[supp_h[i][I[j],:] for i in W[j]],coe_g[J[j]],coe_h[W[j]],v[j],s2k[j],sk[j],sk_g[j],sk_h[j],s2k_g[j],s2k_h[j])
    ...
    for iter in 2:t
        ...
        for j in 1:p
            ...
            #println("Clique ",j,"th: ==================")
            Usupp[j],lUsupp[j],block_g0[j],block_g[j],block_h[j],lblock_g0[j],lblock_g[j],lblock_h[j],lt_block_g0[j],lt_block_g[j],lt_block_h[j]=get_blocks_Cliq_mix_LMBM(k,lI[j],lJ[j],lW[j],Supp[I[j],IndA[j]],lmon_g[J[j]],lmon_h[W[j]],[supp_g[i][I[j],:] for i in J[j]],[supp_h[i][I[j],:] for i in W[j]],coe_g[J[j]],coe_h[W[j]],v[j],s2k[j],sk[j],sk_g[j],sk_h[j],s2k_g[j],s2k_h[j])
            if iter==t
                Usupp[j]=sortslices(Usupp[j],dims=2)
                Usupp[j]=unique(Usupp[j],dims=2)
                lUsupp[j]=size(Usupp[j],2)
...
function get_mex_mix_LMBM(n::Int64,lUsupp::Vector{UInt64},Usupp::Vector{Matrix{UInt64}},p::Int64,I::Vector{Vector{UInt16}},lI::Vector{Int64})
